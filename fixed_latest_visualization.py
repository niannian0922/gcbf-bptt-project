...
def main():
    print('🎯 修复后的最新真实模型可视化生成器')
    print('=' * 70)
    print('📋 基于CBF修复+协作损失训练后的最新模型')
    print('=' * 70)
    ...
    if os.path.exists(policy_path):
        size_mb = os.path.getsize(policy_path)/(1024*1024)
        print(f'📊 策略文件: ✅ {size_mb:.1f}MB')
    else:
        print('❌ 策略文件不存在')
        return False
        
    if os.path.exists(cbf_path):
        size_kb = os.path.getsize(cbf_path)/1024
        print(f'📊 CBF文件: ✅ {size_kb:.1f}KB')
    else:
        print('❌ CBF文件不存在')
        return False
    ...
    try:
        ...
        if isinstance(obs_shape, tuple):
            # 如果observation_shape是tuple，取最后一个维度
            actual_obs_dim = obs_shape[-1] if len(obs_shape) > 1 else obs_shape[0]
        else:
            actual_obs_dim = obs_shape
            
        print(f'✅ 环境创建成功: {env.num_agents}智能体')
        print(f'📊 原始观测形状: {obs_shape}')
    ...
    try:
        # 确保input_dim是整数
        policy_input_dim = int(input_dim)
        policy_output_dim = 2  # 动作维度固定为2D
        ...
        policy_config = {
            ...
            'predict_alpha': True,
            'perception': {
                'input_dim': policy_input_dim,  # 确保是整数
                'hidden_dim': 256,
                'num_layers': 2,
                'activation': 'relu',
                'use_vision': False
        ...
        print(f'🔧 策略配置 - 输入维度: {policy_input_dim}, 输出维度: {policy_output_dim}')
    ...
    # 生成轨迹
    print('\n🎬 开始生成最新模型轨迹...')
    trajectory_data = generate_trajectory(env, policy, device)
    ...
    # 创建可视化
    print('\n🎨 创建最新模型可视化...')
    output_file = create_visualization(trajectory_data, env_config, model_path)
...
def generate_trajectory(env, policy, device):
    """生成最新模型轨迹"""
    print('   🎬 使用最新神经网络策略生成轨迹...')
...
def create_visualization(trajectory_data, env_config, model_path):
    """创建最新模型可视化"""
...
if __name__ == "__main__":
    print('🎯 修复后的最新真实模型可视化系统')
    print('基于CBF修复+协作损失训练后的最新模型生成100%真实可视化')
    print('=' * 80)
    ...
    if success:
        print(f'\n🎉 成功生成最新真实模型可视化!')
        print(f'🎯 这是基于您最新协作训练模型的真实表现')
        print(f'🧠 100% 使用最新真实神经网络策略输出')
        print(f'🔧 包含所有修复: CBF维度修复 + 协作损失功能')
        print(f'📊 反映最新训练成果，不是之前有问题的版本')
    else:
        print(f'\n❌ 最新真实模型可视化失败')
