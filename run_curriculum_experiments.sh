...
# 實驗1: Alpha Medium 預訓練
echo "🔄 開始 Alpha Medium 預訓練..."
python train_bptt.py \
    --config config/alpha_medium_pretrain.yaml \
    --device $DEVICE \
    --log_dir logs/curriculum/alpha_medium_pretrain \
    --seed $SEED
...
# 實驗2: Simple Collaboration 預訓練
echo ""
echo "🔄 開始 Simple Collaboration 預訓練..."
python train_bptt.py \
    --config config/simple_collaboration_pretrain.yaml \
    --device $DEVICE \
    --log_dir logs/curriculum/simple_collaboration_pretrain \
    --seed $SEED
...
# 實驗3: Alpha Medium Fine-tuning
echo "🔄 開始 Alpha Medium Fine-tuning..."
python train_bptt.py \
    --config config/alpha_medium_obs.yaml \
    --device $DEVICE \
    --log_dir logs/curriculum/alpha_medium_finetune \
    --load_pretrained_model_from logs/curriculum/alpha_medium_pretrain \
    --seed $SEED
...
# 實驗4: Simple Collaboration Fine-tuning
echo ""
echo "🔄 開始 Simple Collaboration Fine-tuning..."
python train_bptt.py \
    --config config/simple_collaboration.yaml \
    --device $DEVICE \
    --log_dir logs/curriculum/simple_collaboration_finetune \
    --load_pretrained_model_from logs/curriculum/simple_collaboration_pretrain \
    --seed $SEED
...
python visualize_bptt.py \
    --model_dir logs/curriculum/alpha_medium_finetune \
    --output results/curriculum_alpha_medium_collaboration.gif \
    --device $DEVICE

echo ""
echo "🔄 生成Simple Collaboration課程學習可視化..."
python visualize_bptt.py \
    --model_dir logs/curriculum/simple_collaboration_finetune \
    --output results/curriculum_simple_collaboration.gif \
    --device $DEVICE
